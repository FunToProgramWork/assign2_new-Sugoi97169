...
if(game%2!=0&&game!=0){
   ...
   if(fo==1){foodpos(); }
   if(fo==0){image( food, foodx, foody);}  
    else{foodx=-80;foody=0;}
...
if(ratxb >soldxs && soldxb > ratxs &&
         ratyb > soldys && soldyb > ratys){lifen--;heart();ratx = 320;raty = 80;print(lifen);}
         
if(ratxb >foodxs && foodxb > ratxs &&
         ratyb > foodys && foodyb > ratys){lifen++;heart();fo=2;print(lifen);}
}
...
void  keyPressed(){
  ...
  if (keyPressed) {
    if (keyCode == UP) {
      raty-=5;  image( ratImg, ratx, raty);
    }
    if (keyCode == DOWN) {
      raty+=5; x = -80;y=0;image( ratDow, ratx, raty);
    }
    if (keyCode == LEFT) {
      ratx -= 5;x = -80;y=0;image( ratleft, ratx, raty);
    }
    if (keyCode == RIGHT) {
      ratx += 5;x = -80;y=0;image( ratRight, ratx, raty);
    }
